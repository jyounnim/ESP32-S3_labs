# 03. Vibration/Sensor Anomaly Detection
...
The LIS3DH and the SSD1306 go on **two separate I2C buses** — the ESP32-S3 has two independent I2C controllers (I2C0, I2C1) you can use at the same time. This reuses the same pin assignment as this series' Zephyr IPC lab (the multi-sensor + SSD1306 lab) — if you've already wired it up that way, there's no need to touch the breadboard again.
...
Both `lab/01_data_collect` and `lab/03_inference` include a very small SSD1306 driver at `src/oled_status.c`/`oled_status.h` (128x64, I2C1, 8x8 text). ESP-IDF doesn't have a standard display driver stack like Zephyr's display subsystem, so only the minimum this lab needs (clear the screen + print up to 3 lines of text) was implemented from scratch.
...
- **Step 1 (`01_data_collect`)**: line 1 shows `LAB03 Step1`, line 2 shows `Collecting...`, and line 3 updates live with the number of windows collected so far (`windows: N`).
- **Step 4 (`03_inference`)**: line 1 shows `LAB03 Step4`, line 2 shows `normal` or `** ANOMALY **`, and line 3 updates every inference with the just-computed reconstruction error (`mse=0.0123`) — you can tell at a glance whether it's normal or anomalous right now, just from the screen.

If you'd like to reuse this display in another lab (01/02), just copy the three files `oled_status.h`/`oled_status.c`/`font8x8_basic.h` as-is (assuming the I2C1/GPIO4·5 wiring is already in place).
...
If you've got a camera module or an I2S microphone ready, you can continue on to `01_FACE_DETECTION_LAB` (face detection) or `02_WAKE_WORD_LAB` (wake word detection).
