...
이 랩은 두 단계로 나뉘며, 각 단계가 독립된 PlatformIO 프로젝트입니다 (`03_SENSOR_ANOMALY_LAB`이 `01_data_collect`/`02_train_autoencoder`/`03_inference`로 나뉜 것과 같은 방식).
...
- `noise`/`unknown` 데이터를 충분히 모으지 않으면, 조용한 환경에서도 오탐(false positive)이 잦아집니다 — 실제 사용 환경의 배경 소음을 데이터셋에 포함시키는 게 정확도보다 더 중요한 경우가 많습니다 (`03_SENSOR_ANOMALY_LAB`에서 겪은 "너무 조용한 조건에서 수집한 데이터가 오히려 문제"였던 것과 같은 종류의 함정입니다).
...
`03_SENSOR_ANOMALY_LAB`(진동/센서 이상 탐지, HW-664 모듈/LIS3DH 3축 가속도계 필요, SSD1306 OLED는 선택사항)로 바로 이어서 진행할 수 있습니다. 카메라 모듈을 준비하셨다면 `01_FACE_DETECTION_LAB`(얼굴 인식)으로 진행하셔도 됩니다.
